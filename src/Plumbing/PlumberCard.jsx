...
function PlumberCard({ plumber, isSelected, onClick }) {
  return (
    <div
      onClick={onClick}
      className={`flex items-center gap-4 bg-white border rounded-2xl px-4 py-3.5 hover:border-blue-300 hover:shadow-md hover:shadow-blue-50 transition-all duration-200 cursor-pointer group ${
        isSelected
          ? "border-blue-400 shadow-md shadow-blue-50 ring-1 ring-blue-200"
          : "border-gray-100"
      }`}
    >
      {/* Avatar */}
      <div className={`w-11 h-11 rounded-full flex items-center justify-center text-sm font-semibold shrink-0 transition-colors ${
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-gray-800 truncate">{plumber.name}</p>
        ...
        <p className="text-xs text-gray-400 mt-0.5 flex items-center gap-1">
          ...
          {plumber.areas.length > 1 && (
            <span className="text-gray-300">+{plumber.areas.length - 1} more</span>
          )}
        </p>
      </div>
      ...
      <div className="shrink-0 text-right space-y-1">
        <p className="text-sm font-bold text-gray-800">{plumber.rate}</p>
        <StarRating rating={plumber.rating} />
        <p className="text-xs text-gray-400">{plumber.reviews} reviews</p>
      </div>
